...
#if defined(PLATFORM_ANDROID) || defined(PLATFORM_RPI) || defined(PLATFORM_DRM) || defined(PLATFORM_UWP) || defined(PLATFORM_EMBED)
...
#endif
#if defined(PLATFORM_DESKTOP) || defined(PLATFORM_WEB) || defined(PLATFORM_RPI) || defined(PLATFORM_UWP) || defined(PLATFORM_DRM) || defined(PLATFORM_EMBED)
...
#endif
#if defined(PLATFORM_DESKTOP)||defined(PLATFORM_EMBED)
...
#if defined(PLATFORM_ANDROID) || defined(PLATFORM_RPI) || defined(PLATFORM_DRM) || defined(PLATFORM_EMBED)
    struct timespec ts;
...
static bool InitGraphicsDevice(int width, int height)
{
    ...
    #if !defined(__APPLE__) && !defined(PLATFORM_EMBED)
...
{
    if (action == 0) CORE.Input.Keyboard.currentKeyState[key] = 0;
...
void EmbedWindowDropCallback(int count, const char **paths)
{
    ClearDroppedFiles();
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        strncpy(CORE.Window.dropFilesPath[i], paths[i], strlen(paths[i])-1);
    }
